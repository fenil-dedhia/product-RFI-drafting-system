...
6. **Create the PM roster.** Copy the tracked template `knowledge-base/pm-roster.EXAMPLE.md` to `knowledge-base/pm-roster.md`, then record who owns which product area, the tag to mention each person by, and their contact. The agent uses it to route open questions to the right owner when answering general product questions (Mode B — see [`GENERAL-Q&A-PLAYBOOK.md`](./GENERAL-Q&A-PLAYBOOK.md)), tagging them inline and in a closing open-points table. It is **git-ignored** for the same reason as the disclosure notes — it carries colleagues' names and contact details — so it never travels with a fork and each teammate keeps their own. The ignore rule already ships in `.gitignore`; you don't need to add it. Only the `.EXAMPLE.md` template is tracked. Skip it and the agent will stop and ask you for the roster rather than guessing at ownership.

7. **Give every module a doc source.** `knowledge-base/doc-sources.md` should register a documentation repo or site for **each module** you'd declare as part of your platform. This matters more than it looks: a module with no registered corpus can't be answered on from primary sources, and reasoning about it from a sibling module's docs is how a confident wrong answer gets made. The agent warns at pre-flight if a module in scope has no source, but it's cheaper to fix here.
...
6. **Refresh the PM roster if ownership has changed.** Areas move between people; a tag pointing at someone who no longer owns the area sends open questions nowhere.

7. **Gather the team-only facts** for the interview step: current **version and GA date per module**, **active customer counts**, any **third-party / partner-supplied capability disclosure** wording, the exact **module list that defines "the platform,"** and any **percentage splits** the questionnaire asks for. Add anything **commercial** the questionnaire touches — pricing, packaging, licensing, entitlements, what's sold standalone versus bundled — because the agent is required to escalate every one of those rather than infer it from documentation, and it will ask. No document can answer these; having them ready keeps the run moving.

8. **Plan to be around for _two_ live checkpoints, not one.** The first is the scoping interview above, before drafting. The second comes after drafting: the agent runs a conflict pass over its own answers and asks you to rule on every contradiction the sources can't settle — between its answers and ones already in the questionnaire, between its answers and what last cycle's FINAL told the same analyst, or between two of its own answers. Those rulings are quick (usually "keep the old one," "use the new one," or "keep both, flagged"), but they need you. Whatever you don't get to lands in the reviewer's note as an open list instead — the run never blocks on it.
