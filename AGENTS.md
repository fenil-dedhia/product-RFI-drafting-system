<!-- PAIRED FILE. This is CLAUDE.md, for agents that read AGENTS.md instead.
     The two must be changed together in the same commit. If they disagree, CLAUDE.md
     is the one that was edited and this is the one that rotted. -->

> **Codex and other AGENTS.md readers: raise the document cap before you rely on this file.**
> Codex stops reading once the combined instructions reach `project_doc_max_bytes`, 32 KiB by
> default, and this constitution is a little over that on its own. The tail is the glossary, and
> it is dropped silently with no warning in the transcript. Set a higher value in
> `~/.codex/config.toml` (user-wide) or `.codex/config.toml` (this project):
>
> ```toml
