...
- Target: the blank questionnaire in outputs/blank-current-RFI/. Never edit it in place — write versioned filled copies into outputs/.
...
Work in the order CLAUDE.md defines. Start with the pre-flight check, then extract the cell schema, then build the facts register (provenance or VERIFY for every number — never inherit or fabricate a figure; pin the counting definition for every aggregate). Then stop and interview me on the "Always confirm" items before drafting a single answer. After I answer, draft to the character limits, verify every length and every number by measurement, and write versioned deliverables into outputs/ with the reviewer's note. Do not brand anything _FINAL until I explicitly confirm a version is the final deliverable.
```
